...
void setup() {
  ...
  scale1.begin(3, 4);
  scale1.set_scale(260); // (450)
  scale1.tare();

  scale2.begin(5, 6);
  scale2.set_scale(260); // (450)
  scale2.tare();

  scale3.begin(7, 8);
  scale3.set_scale(218); // (224)
  scale3.tare();
...
void processSensorData() {
  float Moment_1 = (((scale1.get_units() * 0.001) * 1.1983 - 0.0062) * 0.88)*0.947 * MOMENT_TENZ;
  if (Moment_1 < 0.0){
    Moment_1 = 0;
  }
  float Moment_2 = (((scale2.get_units() * 0.001) * 1.1983 - 0.0062) * 0.88)*0.947 * MOMENT_TENZ;
  if (Moment_2 < 0.0){
    Moment_2 = 0;
  }

  float res_moment = (Moment_1 + Moment_2) / 2;
  if (res_moment < 0.0){
    res_moment = 0;
  }
...
void sendStandInfo() {
  Serial.println("Наименование стенда: момент");
